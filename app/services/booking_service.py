...
from app.schemas.booking import BookingAttendanceUpdate, BookingBulkCreateItem
...
class BookingService:
    ...
    def create_my_bulk_bookings(self, current_user: User, payload: List[BookingBulkCreateItem]) -> List[Booking]:
        ...
        if existing_bookings:
            booked_slot_ids = ", ".join(sorted({booking.trainer_slot_id for booking in existing_bookings}))
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
